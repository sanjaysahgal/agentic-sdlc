...
| 3.10 | Thread survives bot restart | General | Restart bot, follow up in slash thread | Agent responds (not concierge) | ✅ 2026-04-24 |
...
| 2026-04-24 | 4.1 | Feature channel routed to PM instead of Architect (regression) | Two competing sources of truth: persisted `confirmedAgent` vs GitHub phase. No invariant test. | `resolveAgent()` — single source of truth. Phase from GitHub always wins. 20 invariant + cross-context tests prevent recurrence. Principle 13 in CLAUDE.md. | ✅ Fixed |
| 2026-04-24 | 5.4 | PM went rogue during escalation — patched spec, escalated new items, offered to finalize | Architect→PM continuation path ran PM with full tools (readOnly=false) | Set readOnly=true on PM continuation path — PM can only respond with prose | ✅ Fixed |
| 2026-04-24 | 4.1 | /pm in feature channel: PM said "spec complete" but 7 deterministic findings exist | PM ran as temporary override without readiness audit pipeline | Pending — PM temporary override needs to run deterministic audit |
