...
| 2026-04-24 | 4.1 | Feature channel routed to PM instead of Architect (regression) | Stale `confirmedAgent=pm` persisted from slash command test + PM phase check only handled one advance | PM path now checks ALL post-PM phases and routes to correct agent | ✅ Fixed |
