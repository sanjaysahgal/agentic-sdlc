# Archon Patent — Non-Provisional Filing Tracker

**Entity status:** Small Entity
**Provisional App No:** 64/015,378
**Provisional Filing Date:** March 24, 2026
**Non-Provisional Deadline:** March 24, 2027

---

## Filing Steps

| # | Step | Cost (Small Entity) | DIY / Attorney | Status |
|---|------|---------------------|----------------|--------|
| 1 | Create/verify USPTO.gov account at Patent Center | $0 | DIY | Done |
| 2 | Confirm small entity eligibility — qualified (independent inventor, no assignments to large entities, income exceeds micro threshold) | $0 | DIY | Done |
| 3 | Prepare specification document | $0 | DIY | Done |
| 3a | — Title of the Invention | — | DIY | Done |
| 3b | — Cross-Reference to Related Applications (cite provisional) | — | DIY | Done |
| 3c | — Field of the Invention | — | DIY | Done |
| 3d | — Background of the Invention (19 competitors, 11 limitations) | — | DIY | Done |
| 3e | — Summary of the Invention (17 bullet points) | — | DIY | Done |
| 3f | — Brief Description of the Drawings (17 figures) | — | DIY | Done (descriptions written, drawings pending) |
| 3g | — Detailed Description of Preferred Embodiments (17 embodiments) | — | DIY | Done |
| 3h | — Abstract | — | DIY | Done |
| 4 | Prepare formal drawings (black & white line art, numbered refs, FIGs 1–17) — generated via Mermaid, SVG + PDF in docs/patent/drawings/ | $0 | DIY | Done |
| 5 | Draft claims (20 total — restructured from 27 to save $3,440) | $0 | DIY | Done |
| 5a | — Independent claims (3: system, method, CRM) | — | DIY | Done |
| 5b | — Dependent claims (17) | — | DIY | Done |
| 6 | Prepare Application Data Sheet (ADS) — completed via Web ADS in Patent Center | $0 | DIY | Done |
| 7 | Prepare Inventor's Oath/Declaration (form PTO/AIA/01) — completed and signed | $0 | DIY | Done |
| 8 | File via Patent Center — App #19/646,571, Confirmation #6659, filed 04/14/2026 | $730 | DIY | Done |
| 8a | — File corrected ADS to add domestic benefit claim (provisional 64/015,378) — blocked by preexam processing validation error. Check back ~April 28, 2026. Priority protected by spec text in the meantime. Also verify entity status shows "Small" not "Regular Undiscounted". | $0 | DIY | TODO — check week of April 28 |
| 9 | Filing confirmation received — App #19/646,571 | $0 | — | Done |
| 10 | Wait for examiner assignment (12–18 months) | $0 | — | Waiting |
| 11 | Receive first Office Action | $0 | — | Waiting |
| 12 | Respond to Office Action (3-month deadline) | $0–$4,000 | DIY or Attorney | Waiting |
| 12a | — If 101 rejection (abstract idea): argue technical improvement | — | Consider Attorney | Waiting |
| 12b | — If 102 rejection (anticipation): distinguish prior art | — | DIY | Waiting |
| 12c | — If 103 rejection (obviousness): argue improper combination | — | DIY | Waiting |
| 12d | — If 112 rejection (clarity): fix claim language | — | DIY | Waiting |
| 13 | Second Office Action / Final (if needed) | $0–$650 | DIY or Attorney | Waiting |
| 13a | — Option A: Response After Final | $0 | DIY or Attorney | — |
| 13b | — Option B: RCE (Request for Continued Examination) | $650 | DIY or Attorney | — |
| 13c | — Option C: Appeal to PTAB | $420 | Attorney recommended | — |
| 14 | Notice of Allowance — pay issue fee (3-month deadline) | $600 | DIY | Waiting |
| 15 | Patent grants | $0 | — | Waiting |

---

## Maintenance Fees (Post-Grant)

| Deadline | Cost (Small Entity) | Status |
|----------|---------------------|--------|
| 3.5 years after grant | $900 | Future |
| 7.5 years after grant | $2,300 | Future |
| 11.5 years after grant | $3,800 | Future |

---

## Cost Summary

| Category | Estimated Cost |
|----------|---------------|
| Filing fees (step 8) | $730 (paid) |
| Office Action response (if attorney for 101) | $0–$4,000 |
| RCE if needed | $650 |
| Issue fee | $600 |
| **Total through grant** | **$730–$5,980** |
| Maintenance (full 20 years) | $7,000 |
| **Lifetime total** | **$7,730–$12,980** |

---

## Key Dates

| Event | Date |
|-------|------|
| Provisional filed | March 24, 2026 |
| Non-provisional filed | April 14, 2026 |
| Non-provisional App # | 19/646,571 |
| Corrected ADS (domestic benefit claim) | TODO — check week of April 28, 2026 |
| First Office Action expected | ~June–October 2027 |
| Office Action response deadline | — |
| Notice of Allowance | — |
| Issue fee deadline | — |
| Patent grant date | — |
| Maintenance fee 1 due | — |
| Maintenance fee 2 due | — |
| Maintenance fee 3 due | — |

---

## Case Law for 101 Defense

Keep these handy for Office Action responses:
- **Enfish v. Microsoft** — claims directed to specific improvement in computer capabilities (self-referential table)
- **BASCOM v. AT&T** — inventive concept in ordered combination of elements
- **DDR Holdings v. Hotels.com** — solution necessarily rooted in computer technology
- **Core Wireless v. LG** — improved user interface functionality
