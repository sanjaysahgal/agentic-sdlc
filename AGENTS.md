...
**Render ambiguity + copy completeness audit.** After every spec save, `auditSpecRenderAmbiguity(designSpec)` in `spec-auditor.ts` runs. It has three stages: (1) deterministic screen-reference check — scans `## User Flows` for screen/sheet/modal names and verifies each has a definition in `## Screens`; (2) `auditCopyCompleteness(designSpec)` — deterministic copy check that extracts all quoted literals and flags: sentence-case multi-word strings missing terminal punctuation (`.`, `!`, `?`) and placeholder brackets (`[TBD]`, `[placeholder]`, etc.); (3) Haiku call for defined-but-vague elements: relative positioning, missing animation timing, undefined interactive copy, and linguistically incomplete sentences. Result returned as `renderAmbiguities: string[]`. **Blocking** — when non-empty, the design agent must call `apply_design_spec_patch` in the same response, resolving every item. Copy completeness is treated identically: a tagline without a terminal period or `[TBD]` in any copy literal blocks approval. The design agent owns the copy: it resolves all copy issues using design judgment — it does not ask the user unless the gap requires a product decision.
