...
**HTML preview always full-regeneration:** `apply_design_spec_patch` always calls `generateDesignPreview` with the full merged spec — no patch-based surgical update. `updateDesignPreview` was removed from the save path: it caused content misses (Sonnet failed to apply patch text) and regressions on out-of-spec elements (Auth Sheet animation direction changed when an unrelated tagline patch was processed).

**Readiness query → platform audit injection:** When the user asks a readiness question ("is this ready to hand off to engineering?", "ready for engineering?", etc.), `isReadinessQuery()` (deterministic keyword match, no LLM) detects it. The platform immediately runs `auditPhaseCompletion(DESIGN_RUBRIC)` and injects findings as `[PLATFORM READINESS AUDIT]` into the enriched user message before the agent runs. The design agent sees the findings and must surface each one: design gaps with its own recommendations, product gaps via `offer_pm_escalation`, architecture gaps via `offer_architect_escalation`. This is platform-enforced — not prompt-rule-dependent.

**HTML renderer structural guarantees:** `sanitizeRenderedHtml()` applies 5 deterministic post-generation fixes: (1a/b) hero `x-show` → `:class`, (1c) hero missing `id="hero"` → inject id + convert x-show to :class, (2) thread missing `display:none` or `x-show` → inject both. `validateRenderedHtml()` runs two BLOCKING checks: hero must have `id="hero"`, and hero must not be nested inside thread. Either blocking issue triggers one LLM retry. If still blocking after retry, an error is thrown — the preview is not silently incorrect. The render ambiguity auditor (`auditSpecRenderAmbiguity`) also checks chip position anchor (horizontal row without a fixed-element anchor is ambiguous) and SSO button icon+text internal layout (icon/text arrangement must be specified).
...
**Brand token drift detection (color + animation).** `brand-auditor.ts` runs on every response (state query and agent run). It diffs every CSS variable token in the spec's Brand section against BRAND.md canonical values (`auditBrandTokens()`) and also checks animation parameters — glow duration, blur radius — against BRAND.md's `## Animation` section (`auditAnimationTokens()`). Both run as pure string diffs — zero API cost, zero latency. All drift (color + animation) is merged into a single PLATFORM NOTICE and injected into the enriched user message. Never silently corrects values without surfacing the drift first.
