...
**Agent discoverability:** The concierge directs users to slash commands (`/pm`, `/design`, `/architect`) for product-level discussions (vision, brand, architecture). Slash commands work at the top level of channels only — inside threads, use `@pm:`, `@design:`, or `@architect:` text prefix. Feature-scoped work goes to `#feature-*` channels. **Intent over identity:** when a user states a clear intent, the concierge acts on it immediately rather than asking for their role first. **No company names or "AI" prefix:** agents are described by capabilities, not credentials.
