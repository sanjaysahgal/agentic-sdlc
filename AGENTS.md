...
**Post-response uncommitted decisions audit:** After every design agent text-only response (no save tool called), if conversation history > 6 messages, the platform runs `identifyUncommittedDecisions`. If uncommitted decisions exist, the platform appends a note to the Slack message prompting the user to "save those". The agent handles "save those", "save them", "commit those", and "lock those in" by immediately calling `apply_design_spec_patch` with all uncommitted decisions — no clarifying question asked.
...
**Render ambiguity audit.** After every spec save, `auditSpecRenderAmbiguity(designSpec)` in `spec-auditor.ts` runs as an independent Haiku call. It identifies elements too vague for consistent rendering: screens with no title/subtitle defined, positions described only as relative ("near the bottom" without pixel spacing), sheets/modals with no entry direction, interactive element text not defined, vague animation ("smooth transition" without timing values). Result is returned in the tool result as `renderAmbiguities: string[]`. When non-empty, the design agent must call `apply_design_spec_patch` in the same response turn to resolve all items — no deferral, no user question. A spec with unresolved render ambiguities is an incomplete spec.
