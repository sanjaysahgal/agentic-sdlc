...
**State response schema (four ordered sections).** `buildDesignStateResponse()` always returns in the same priority order: (1) `*── PENDING ──*` — always shown; uncommitted conversation decisions appear here; (2) `*── DRIFT (spec vs BRAND.md) ──*` — color and animation drift, gates approval; (3) `*── QUALITY ──*` — deterministic design quality issues from `auditRedundantBranding()` + `auditCopyCompleteness()`, gates approval; (4) `*── SPEC ──*` — committed decisions, blocking/non-blocking questions, spec gap, preview link. The CTA is conditional in priority order: uncommitted > drift > quality > blocking > all-clear. `*fix quality*` triggers the agent to patch the spec. Approval (`*approved*`) is never offered while any gate is open.
