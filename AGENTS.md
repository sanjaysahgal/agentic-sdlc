...
**Spec format (`<feature>.design.md`):** Figma link, Design Direction, Brand (tokens + typography), Screens (purpose / states / interactions / notes per screen), User Flows (one per user story), Accessibility decisions, Open Questions. **Open Questions constraint:** every open question must be tagged `[type: engineering]` — product-scope questions must NEVER be written into the design spec as open questions. If a product gap is discovered, call `offer_pm_escalation` immediately. Draft/approval mechanics (auto-save, freeze on approval) wired in Step 3c.
