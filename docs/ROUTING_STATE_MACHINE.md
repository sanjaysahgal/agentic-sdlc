...
22. **I22 — Dismiss-escalation-fall-through.** `RoutingDecision` variant `dismiss-escalation-fall-through` triggered when the user gives an explicit "leave as-is / abandon escalation / spec is fine" signal during an active `pendingEscalation` OR `escalationNotification`. **Detection** is Haiku free-text classifier (`runtime/routing/dismiss-classifier.ts`, `claude-haiku-4-5-20251001`) — never keyword matching, per the project's free-text classification rule. The dispatcher pre-classifies the user message via `classifyDismissIntent(rawText)` and propagates the boolean through `RoutingIntent.dismissIntent` (kept on the intent so the router stays pure — Principle 12). The router branches deterministically on that flag: `dismissIntent === true && pendingEscalation` → `dismiss-escalation-fall-through` with `clear-pending-escalation`; `dismissIntent === true && escalationNotification` → same with `clear-escalation-notification`. Origin agent resumes downstream WITHOUT writeback. **Conservative bias** in the classifier (return NOT-DISMISS on ambiguity): false-dismiss is worse than false-hold because the user loses agency on an explicit confirmation; the audit signal itself is preserved either way. **Audit signal preservation:** the deterministic upstream-spec audit (Principle 11) re-fires on the next downstream finalize because the input (the spec content) hasn't changed; the user has agency to dismiss but the system surfaces the same gaps next cycle until they're either patched or marked as audit-exceptions (separate backlog item: "Audit-exception markers — durable false-positive resolution"). **Producer/consumer test rule:** the classifier ships with both a consumer test (mocked Anthropic, asserts parser handles `DISMISS` / `NOT-DISMISS` correctly) AND a producer-prompt test (asserts the system prompt contains the gated tokens + canonical examples for both classes), per the producer-consumer chain rule. Real-fixture-sourced producer test queued as follow-up after Phase 4 cutover when live runs become routine.
...
| E1    | ux-design      | target=pm         | —                      | —               | "leave it as-is" (dismiss) | dismiss-escalation-fall-through(reason=user-dismissed)              | I22 |
| E1    | ux-design      | target=pm (no productSpec) | —             | —               | "yes"                    | invalid-state(reason=missing-productSpec)                             | FLAG-5 |
...
| E1    | ux-design      | —                 | target=pm, origin=design | —             | "leave it as-is" (dismiss) | dismiss-escalation-fall-through(reason=user-dismissed)              | I22 |
...
| E1    | architect      | target=pm         | —                      | —               | —                     | "spec is fine, skip this" (dismiss) | dismiss-escalation-fall-through(reason=user-dismissed) | I22 |
| E1    | architect      | target=pm (no productSpec) | —             | —               | —                     | "yes"                  | invalid-state(reason=missing-productSpec)                 | FLAG-5 |
...
| FLAG-E ✅ | `pendingEscalation.productSpec` was optional but read unchecked when the PM brief was assembled — pre-Phase-5 records that omitted it produced briefs with `undefined` spliced inline. **Fixed Phase 5 (2026-04-28):** v2 router emits `invalid-state(reason=missing-productSpec)` with `clear-pending-escalation` cleanup whenever `targetAgent === "pm"` and `productSpec` is empty / missing; `scripts/migrate-routing-state-v2.ts` drops on-disk pm-target records that lack a non-empty productSpec. Non-pm targets are exempt. Type-level closure (discriminated `PendingEscalation` union) deferred to Phase 6. | §8.3, §8.5 | FLAG-5 — typed required at routing boundary (DONE) |
