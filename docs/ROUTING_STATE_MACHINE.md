...
7. **I7 — Hold-message template derived from registry, posture-coherent.** Replaces hardcoded literals AND third-party-agent personification. Today's text "Design is paused — say yes to bring the PM into this thread" has three bugs: (a) FLAG-C hardcoded "Design"; (b) "bring the PM into this thread" personifies the agent and breaks when the human user is the PM stakeholder; (c) fires even when the user typed `/pm` (FLAG-A — fixed by I1, removing the case entirely). Phase 5 replacement template (fires only on non-slash messages while held): `"<DownstreamPhase> on <featureName> is blocked by <N> unresolved item<s> in the <upstreamSpecType> spec: <items>. Reply 'yes' to have the <heldAgent> draft tightenings, or send your own corrections directly."` All fields registry-derived.
...
21. **I21 — Orientation-on-resume (Phase 5).** When PM/Designer/Architect runs in escalation-engaged mode (with a brief, via `/pm` confirmation OR `yes` confirmation), the response MUST lead with a one-paragraph orientation block before recommendations: (1) name the current spec state ("the product spec for `<feature>` is approved on main"), (2) name who escalated and why ("during engineering, the architect ran the upstream-spec audit and flagged N items"), (3) link the spec, (4) state the agent's mode, then transition to items + recommendations. **Voice rules:** first-person agent, second-person user, no passive constructions like "you're being engaged" — the engagement is implicit in the agent responding. Platform-enforced via a structural gate (response missing the orientation header is rejected and re-run with an enforcement override; same pattern as `enforceOpinionatedRecommendations` in `runtime/spec-auditor.ts`). Same template applies to designer (when pulled by architect) and architect (when pulled by either).
