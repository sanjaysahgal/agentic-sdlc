...
| **State response quality gate** | `agents/design.ts` → `buildDesignStateResponse({ qualityIssues })` + state query path in `message.ts` | Quality issues rendered as `*── QUALITY ──*` section on every state query. CTA priority: uncommitted > drift > quality > blocking — approval blocked until all clear. | None (deterministic results pre-computed before `buildDesignStateResponse` call) |
