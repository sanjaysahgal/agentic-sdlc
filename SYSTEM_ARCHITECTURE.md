...
| Renderer structural validation | `runtime/html-renderer.ts` → `sanitizeRenderedHtml()` + `validateRenderedHtml()` | Hero not behind `x-show` (sanitized); hero missing `id="hero"` → inject id + convert x-show to :class (Fix 1c); thread has `display:none` + `x-show` (sanitized — both injected if absent); inspector buttons have static resting styles (sanitized); no unescaped apostrophes in JS strings (sanitized); hero must have `id="hero"` (BLOCKING — retry if absent); hero must not be nested inside thread (BLOCKING — bracket-counter walks thread's paired `</div>` to scope the check precisely, no false positives from sibling hero; retry if violated); keyframe animations present; body has explicit background-color | None (pure string/regex match + one LLM retry on blocking failures) |
| Readiness query audit injection | `runtime/agent-router.ts` → `isReadinessQuery()` (Haiku classifier); `runtime/phase-completion-auditor.ts` → `auditPhaseCompletion(DESIGN_RUBRIC)` | Platform runs the design phase completion audit when user asks a readiness question ("can we ship?", "good to go?", "should I tell the engineer?" etc.) — injected as `[PLATFORM READINESS AUDIT]` into enriched user message before agent runs. Design gaps → agent recommends fix. Product gaps → `offer_pm_escalation`. Architecture gaps → `offer_architect_escalation`. | Haiku (intent classification, same pattern as isOffTopicForAgent/isSpecStateQuery); Sonnet (completion audit) |
| Design escalation triage | `agents/design.ts` → `DESIGN_TOOLS` (`offer_pm_escalation` + `offer_architect_escalation`); `runtime/conversation-store.ts` → `PendingEscalation.targetAgent: "pm" \| "architect"` | Product behavior questions → PM. Architecture/data design questions → Architect. Visual definition gaps agent can resolve → agent owns it with a concrete proposal. | None (agent judgment guided by system prompt triage rules) |
| Render ambiguity chip + SSO audit | `runtime/spec-auditor.ts` → `auditSpecRenderAmbiguity()` | Chip rows must name a fixed-element anchor (not just "horizontal row"); SSO/auth buttons with icon + label must specify internal horizontal arrangement | Haiku (save-time) |
...
**HTML preview renderer (`runtime/html-renderer.ts`):** Uses `claude-sonnet-4-6` (not Haiku) for production-quality rendering. When `context.brand` is available, it is passed as a separate `brandContent` parameter and prepended to the user message as an `AUTHORITATIVE BRAND TOKENS` block — the renderer reads color and animation values from this block, not the spec's Brand section (which may have drifted). Returns `{ html: string, warnings: string[] }` — `warnings` contains structural validation results (missing keyframes, background token not applied, truncation, missing explicit body background-color) that are surfaced as a tool result field. **No hex color values are hardcoded** in the renderer; all values come from BRAND.md at runtime. The system prompt enforces that body `background-color` and `color` are always in the `<style>` tag — Tailwind CDN custom classes (`bg-primary`, `text-fg`) fail silently on `file://` URLs and Slack attachments because the CDN may not load before browser paint. The system prompt requires Alpine.js state and methods to be declared in a `<script>` block using the `x-data="functionName()"` pattern — `$nextTick` and other Alpine magic properties are not safe inside HTML attribute strings. **Phone content area structure is mandated:** the phone's content zone uses `flex:1; position:relative; overflow:hidden` as a stacking context; the hero div uses `position:absolute; inset:0` with `:class` hiding (NOT `x-show`) — hero is always visible before Alpine loads; the thread div uses `style="display:none"` + `x-show` — hidden before Alpine, shown reactively when msgs exist. **JS string safety is mandated:** all string literals in `appData()` must use double quotes — single-quoted strings containing apostrophes (e.g. `'How's my rate?'`) cause a JS syntax error, Alpine throws on `x-data="appData()"`, and all `x-show` elements stay `display:none` permanently → blank phone screen. **Inspector button base styles are mandated:** full resting appearance (background, color, border) must be in the static `style=` attribute; `:style` bindings only add active-state highlights. `sanitizeRenderedHtml()` (exported; also used in `updateDesignPreview`) deterministically fixes hero x-show → :class, injects thread display:none, and converts apostrophe-containing single-quoted JS strings to double-quoted. `validateRenderedHtml()` then checks for remaining blocking issues (truncated HTML, JS syntax errors) and non-blocking warnings. Blocking issues trigger one retry with error context injected; if still blocking after retry, throws. The rendered preview uses a phone frame (390×844) + inspector panel layout: the phone frame contains the fully interactive app experience; the inspector panel lists all spec states as clickable buttons that directly apply a state to the phone frame without requiring click-through. **The renderer is the agent** — HTML preview files are never hand-written. `scripts/generate-preview.ts` is the CLI entry point that reads the spec and brand files from GitHub, calls `generateDesignPreview()`, and writes the output via `fs.writeFileSync()`.
...
- **Layer 2 (full regeneration on save):** `apply_design_spec_patch` calls `saveDesignDraft(mergedSpec)`. `saveDesignDraft` always calls `generateDesignPreview()` with the full merged spec — no patch-based update. `updateDesignPreview()` was removed from this path because it caused two failure modes: (1) Sonnet failed to apply patch text correctly (missed or paraphrased changes), and (2) Sonnet modified elements outside the patch scope, regressing previously-correct animations and screen definitions. Full regeneration from the authoritative merged spec is deterministic and correct.
