...
**Product spec writeback on escalation confirmation (PM path):** When `escalationNotification.recommendations` is set and `targetAgent !== "architect"`, `patchProductSpecWithRecommendations` (`runtime/pm-escalation-spec-writer.ts`) runs before the design agent resumes. It reads the approved product spec from main, calls Haiku to generate a targeted patch, applies the patch with `applySpecPatch`, and saves back to main via `saveApprovedSpec`. The Haiku patch prompt enforces seven rules: (1) REPLACE vague criteria in-place — find the acceptance criterion containing vague language ("soft", "non-intrusive", "ambient", "proactively", "seamlessly", "minimal", "appropriate", "subtle") and replace it with the PM's concrete decision; (2) keep all existing concrete criteria; (3) STRIP visual/design details from PM recommendations before writing (hex values, rgba, component choices — the spec encodes WHAT the user experiences, not HOW it looks); (4) add new concrete criteria for gaps not previously in the spec; (5) route product decisions to `## Acceptance Criteria`, error/failure modes to `## Edge Cases`; (6) HYGIENE PASS — after applying rules 1–5, scan the entire spec for any remaining vague language; if the correct concrete meaning can be inferred from PM decisions already present in the spec (current or prior), replace it; if meaning cannot be inferred, leave it unchanged for the next escalation (no invented decisions); (7) output the complete section body so `applySpecPatch` can replace the entire section. After writeback, the PM agent posts the spec-update message ("Product spec updated with confirmed decisions. Design team can continue.") — PM owns this message, not the design agent. This closes the gap permanently — vague criteria are removed, not preserved alongside concrete ones, so criterion 10 does not re-fire on the same language in the next design run.
...
| **PM design-readiness gate** | `runtime/phase-completion-auditor.ts` → `auditPhaseCompletion(PM_DESIGN_READINESS_RUBRIC)` inside `finalize_product_spec` handler | Runs at finalization after the PM_RUBRIC completion gate. Checks that every user-visible behavior is specific enough for a designer to implement without inventing answers: vague sensory/quality descriptors ("ambient", "soft", "subtle"), missing numeric thresholds (session TTL, timeout duration), underspecified error UI behaviors ("handle gracefully", "show an error" without UI treatment), undefined scope terms. Complements PM_RUBRIC criterion 2 (measurable criteria) — catches design-blocking vagueness that Sonnet may pass in PM context but the designer would encounter immediately. Root cause of the 2026-04-13 incident: PM said "nothing blocking" after PM_RUBRIC passed, design agent immediately found 2 PM-GAP items. | **Sonnet** (runs once inside `finalize_product_spec` handler; blocks save if any findings) |
