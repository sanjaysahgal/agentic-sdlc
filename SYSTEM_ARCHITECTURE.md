...
| `[ESCALATION-GATE]` | `interfaces/slack/handlers/message.ts` | Architect pre-run upstream audit result (`ARCHITECT_UPSTREAM_PM_RUBRIC`), finding count, escalation trigger/skip |
...
| **Phase entry upstream audit — Architect** | `message.ts` → `auditPhaseCompletion(ARCHITECT_UPSTREAM_PM_RUBRIC)` + `auditPhaseCompletion(buildDesignRubric(targetFormFactors))` on approved upstream specs, in parallel | PM spec: only product-scope gaps (missing error paths, open questions) — 2 criteria. Design spec: full design rubric. Data requirements, measurability, architecture consistency, non-goals are the architect's own responsibility — not escalated upstream. | **Sonnet** (runs on every architect agent message; same content-addressed caching; combined notice injected if any findings) |
