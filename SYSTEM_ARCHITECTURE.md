...
**Extracted tool handlers** | `runtime/tool-handlers.ts` | PM and architect tool handlers extracted from `message.ts` closures into standalone functions with typed context (`ToolHandlerContext`) and injected dependencies (`PmToolDeps`, `ArchitectToolDeps`). Each handler is independently unit-testable without E2E routing ceremony. The dispatch functions (`handlePmTool`, `handleArchitectTool`) are drop-in replacements wired in `message.ts`. Design tool handler extraction is pending (complex mutable state: `patchAppliedThisTurn`, `lastGeneratedPreviewHtml`). Pattern: all future agents follow the same extraction — handler logic in `tool-handlers.ts`, wiring in `message.ts`. | None (pure refactor — no behavioral change) |
