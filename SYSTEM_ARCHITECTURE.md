...
| Readiness query audit injection | `runtime/agent-router.ts` → `isReadinessQuery()` (keyword match); `runtime/phase-completion-auditor.ts` → `auditPhaseCompletion(DESIGN_RUBRIC)` | Platform runs the design phase completion audit when user asks a readiness question ("ready to hand off to engineering?" etc.) — injected as `[PLATFORM READINESS AUDIT]` into enriched user message before agent runs. Design gaps → agent recommends fix. Product gaps → `offer_pm_escalation`. Architecture gaps → `offer_architect_escalation`. | Sonnet (completion audit); isReadinessQuery is deterministic (no LLM) |
