...
**HTML preview renderer (`runtime/html-renderer.ts`):** Fully deterministic — no LLM call. `renderFromSpec(specContent, brandMd, featureName)` parses the spec for structured values (app name, tagline, placeholder, chips ≤3) and BRAND.md for color tokens and glow animation params, then fills a fixed Alpine.js HTML template. The template always produces correct structure by construction — no sanitizer patches, no retry loops. **No hex color values are hardcoded**; all values come from BRAND.md at runtime. Chip text is passed via `data-chip` attributes (not inline JS strings) to safely handle apostrophes without syntax errors. **Phone content area structure is guaranteed by the template:** hero div uses `position:absolute; inset:0` with `:class` visibility hiding (never `x-show`) — visible before Alpine loads; thread div has `style="display:none"` + `x-show` — hidden before Alpine, shown reactively. Chips are in a horizontal flex row anchored at the bottom of the hero via `margin-top:auto`. Inspector buttons have full static `style=` attributes (background, color, border) so they render correctly before Alpine initializes. `validateRenderedHtml()` checks for blocking structural issues — hero present, hero is sibling (not child) of thread, hero uses `:class`, thread has `display:none` + `x-show`, body has explicit background-color. `generateDesignPreview()` is a thin async wrapper around `renderFromSpec` for backward compatibility. `scripts/generate-preview.ts` is the CLI entry point: reads spec + brand files from GitHub, calls `renderFromSpec`, writes output via `fs.writeFileSync()`.
