...
**Product spec writeback on escalation confirmation (PM path):** When `escalationNotification.recommendations` is set and `targetAgent !== "architect"`, `patchProductSpecWithRecommendations` (`runtime/pm-escalation-spec-writer.ts`) runs before the design agent resumes. It reads the approved product spec from main, calls Haiku to generate a targeted patch, applies the patch with `applySpecPatch`, and saves back to main via `saveApprovedSpec`. `max_tokens: 4096` — bumped from 1024 to prevent silent truncation when many recommendations are applied in one round. The Haiku patch prompt enforces nine rules: (1) REPLACE vague criteria in-place — find the acceptance criterion containing vague language ("soft", "non-intrusive", "ambient", "proactively", "seamlessly", "minimal", "appropriate", "subtle") and replace it with the PM's concrete decision; (2) keep all existing concrete criteria; (3) STRIP visual/design details AND UI copy from PM recommendations before writing — hex values, rgba, component choices, AND exact wording/strings are all stripped; (4) add new concrete criteria for gaps not previously in the spec; (5) route product decisions to `## Acceptance Criteria`, error/failure modes to `## Edge Cases`; (6) RESOLVE contradictions — if two PM recommendations conflict on the same criterion, use the more restrictive one and remove any criterion that contradicts it; never output two criteria that say opposite things on the same topic; (7) COMPLETE or REMOVE incomplete criteria — if any criterion is incomplete (ends mid-sentence, contains TBD/TODO/placeholder), complete it using the PM's confirmed recommendations or remove it entirely; (8) HYGIENE PASS — after applying rules 1–7, scan the entire spec for any remaining vague language; if the correct concrete meaning can be inferred from PM decisions already present in the spec (current or prior), replace it; if meaning cannot be inferred, leave it unchanged for the next escalation (no invented decisions); (9) output the complete section body so `applySpecPatch` can replace the entire section.
