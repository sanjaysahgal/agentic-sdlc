...
>
> **Phase 3 landed (2026-04-27):** Dual-run shadow mode shipped in two stages. **Stage 1** is pure infrastructure: `scripts/generate-shadow-corpus.ts` reads `docs/ROUTING_STATE_MACHINE.md` and writes one JSON fixture per spec row to `tests/fixtures/shadow-corpus/` (54 fixtures today); `scripts/shadow-coverage-driver.ts` (`npm run shadow:coverage`) replays each fixture through the pure routers and asserts zero divergences; `scripts/shadow-coverage-report.ts` (`npm run shadow:report`) parses production `[ROUTING-V2-DIVERGENCE]` lines from `logs/bot-YYYY-MM-DD.log` and aggregates by cell; `.github/workflows/shadow-coverage.yml` runs the driver nightly + on PR. **Stage 2** wires the shadow into production: `runtime/routing/shadow.ts` exposes `logShadowProposalForFeature`/`logShadowProposalForGeneral`, called once at the entry of `handleFeatureChannelMessage` and `handleGeneralChannelAgentMessage`. Each invocation builds a `RoutingInput` from current state, runs the new pure router, and emits a `[ROUTING-V2-PROPOSED] feature=<x> phase=<y> entry=<z> kind=<k> agent=<a> mode=<m>` log line. Fire-and-forget; production behavior is unchanged. The phase fetch in the feature handler is shared with `resolveAgent` (via a new optional `prefetchedPhase` parameter) so the call count to GitHub stays at 1 per message. Coverage gate (a) — synthesized corpus 100% green — passing locally and via CI. Gates (b) and (c) — zero divergences over 48h prod + 3 nights green — accumulate over time.
