...
**Re-audit after escalation writeback (2026-04-25):** `verifyEscalationResolution(blockingSpec, updatedSpec, requestingAgent, targetFormFactors?)` (`runtime/escalation-orchestrator.ts`) runs after every `patchProductSpecWithRecommendations` or `patchDesignSpecWithRecommendations` call in escalation reply paths. Uses the patched spec returned by the writeback function — deterministic re-audit with `auditPmSpec()` or `auditDesignSpec()`. If findings remain: platform sets a new `pendingEscalation` with a fresh categorized brief and does NOT resume the downstream agent. The user sees "Spec partially updated, but N gaps remain. Say *yes* to bring the [agent] back." Wired in two paths: (1) design→PM escalation reply (`branch=escalation-reply`), (2) architect→PM/Design escalation reply (`branch=arch-upstream-revision-reply`). Both paths clear the `escalationNotification` and return early on re-escalation.
