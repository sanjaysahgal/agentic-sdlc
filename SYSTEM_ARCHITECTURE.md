...
**Routing-state migration script (Phase 5 / I2 + I8, 2026-04-28):** `scripts/migrate-routing-state-v2.ts` is a one-time/idempotent cleaner for `.conversation-state.json`. It drops any `pendingEscalation` whose `targetAgent` isn't a valid `AgentId`, plus any `escalationNotification` whose `targetAgent` OR `originAgent` is missing or corrupt (the legacy `"design"` alias is preserved — the v2 router canonicalizes it to `"ux-design"` at read time). Dry-run by default; `--write` persists with a timestamped backup at `<file>.pre-v2-migration.<ts>.bak`. Re-running on an already-clean file is a no-op (`changed: false` in the report). The pure function `migrateRoutingState(parsed)` is exported for tests. Combined with the v2 router's runtime `invalid-state` decision for any corrupt value that slips through after migration (closes FLAG-B for pendingEscalation and FLAG-D for escalationNotification), I2 + I8 are enforced at two layers — disk-side scrub + runtime guard — without depending on production handlers being updated. The full type-level closure (narrowing `EscalationNotification.originAgent` from `"design" | "architect" | undefined` to `AgentId`) is queued for Phase 6 alongside the production-handler cleanup.
