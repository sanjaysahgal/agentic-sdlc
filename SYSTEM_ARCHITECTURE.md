...
4. **Haiku classifier** (`runtime/pm-gap-classifier.ts`) — final safety net; sends agent response to Haiku with a focused system prompt that classifies each item as PM-scope or architect-scope. **PM identity framing:** the prompt defines the PM as owning the WHAT (customer journey, user delight, retention, revenue) — never the HOW. Decision rule: "Can this be answered by deciding what the user experiences, without designing an implementation? YES → PM-scope (GAP:). NO → architect-scope (ARCH:)." Concrete architecture-scope exclusions include session store schema, account-linking mechanism, TTL enforcement, data model design — preventing implementation questions from routing to PM. Returns `GAP: <sentence>` and/or `ARCH: <sentence>` lines, or `NONE`. **Gates 2 and 3 also filter through this classifier before storing** — so non-PM items are stripped regardless of which gate fires. Fires as standalone gate only when no escalation is pending, no spec was saved, and agent is not seeking confirmation.

**Zero-PM-gap rejection (Gates 2 and 3):** If the classifier returns 0 PM-scope gaps, the platform rejects the escalation entirely. Gate 2 (tool call): returns a `REJECTED` message to the agent directing it to resolve brand/design conflicts from BRAND.md or route to architect — no `pendingEscalation` stored. Gate 3 (prose): suppresses without storing. This prevents design/brand conflicts (hex values, animation durations, spec-section conflicts) from incorrectly reaching the PM. Real case Apr 2026: agent called `offer_pm_escalation` for glow opacity/duration/hex conflicts — classifier returned 0 PM gaps correctly, but the platform fell back to `rawQuestion` and stored it. Fix: 0-gap response from classifier aborts escalation at both gates.

**Gate 2 architect-scope pre-seeding:** When Gate 2's Haiku classifier identifies `ARCH:` items (architect-scope) in an `offer_pm_escalation` call, those items are written to the engineering spec draft as `[open: architecture] <question>` entries via `preseedEngineeringSpec` (`runtime/github-client.ts`). The draft is created on the `spec/{feature}-engineering` branch if it doesn't exist. This is a silent platform action — no user-facing message. The architect encounters these pre-seeded questions when the engineering phase begins. Real incident Apr 2026: design agent raised 3 blocking gaps; classifier correctly identified 2 as architect-scope (session storage, conversation data fields) but they were silently dropped. Fix: classifier now returns `architectItems[]` alongside `gaps[]`, and Gate 2 pre-seeds them.
...
**Product spec writeback on escalation confirmation:** When `escalationNotification.recommendations` is set, `patchProductSpecWithRecommendations` (`runtime/pm-escalation-spec-writer.ts`) runs before the design agent resumes. It reads the approved product spec from main, calls Haiku to generate a targeted patch (`##` sections only, concrete measurable entries committed to one specific fix — no alternatives, no "or"), applies the patch with `applySpecPatch`, and saves back to main via `saveApprovedSpec`. This closes the gap permanently: without writeback, the spec auditor re-discovers the same PM gaps on every design run because criterion 10 compares design decisions against the product spec, and the product spec still contains the vague/missing language that triggered the escalation.
