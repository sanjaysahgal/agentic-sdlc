...
| Renderer structural validation | `runtime/html-renderer.ts` → `validateRenderedHtml()` | Hero not behind `x-show`; thread has `display:none`; no unescaped apostrophes in JS strings; keyframe animations present; body has explicit background-color in `<style>` | None (pure string/regex match) |
