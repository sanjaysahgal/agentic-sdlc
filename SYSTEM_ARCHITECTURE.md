...
| **PM design-readiness gate** | `runtime/phase-completion-auditor.ts` → `auditPhaseCompletion(PM_DESIGN_READINESS_RUBRIC)` inside `finalize_product_spec` handler | Runs at finalization after the PM_RUBRIC completion gate. 5 criteria: (1) VAGUE LANGUAGE — sensory/quality descriptors, missing thresholds, transition vagueness ("without disruption"), underspecified error UI; (2) INTERACTION COMPLETENESS — every tappable element defines tap behavior or explicitly states non-interactive; (3) ERROR AND FAILURE RECOVERY — every failure mode names a recovery path; (4) UI MODALITY AND PLACEMENT — every notification/nudge specifies modality, dismissibility, persistence; (5) LOADING AND TRANSITION STATES — every async operation defines loading treatment. Updated after second incident (2026-04-13) where first version (1 criterion) missed 4 of 6 design agent findings. | **Sonnet** (runs once inside `finalize_product_spec` handler; blocks save if any findings) |
