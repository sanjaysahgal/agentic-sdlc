...
| Renderer structural validation | `runtime/html-renderer.ts` → `sanitizeRenderedHtml()` + `validateRenderedHtml()` | Hero not behind `x-show` (sanitized); thread has `display:none` (sanitized); inspector buttons have static resting styles (sanitized); no unescaped apostrophes in JS strings (sanitized; blocking validation triggers retry if remains); keyframe animations present; body has explicit background-color | None (pure string/regex match + one LLM retry on blocking failures) |
