...
**Routing-state migration script (Phase 5 / I2, 2026-04-28):** `scripts/migrate-routing-state-v2.ts` is a one-time/idempotent cleaner for `.conversation-state.json`. It drops any `pendingEscalation` or `escalationNotification` whose `targetAgent` isn't a valid `AgentId` (the legacy `"design"` alias is preserved — the v2 router canonicalizes it to `"ux-design"` at read time). Dry-run by default; `--write` persists with a timestamped backup at `<file>.pre-v2-migration.<ts>.bak`. Re-running on an already-clean file is a no-op (`changed: false` in the report). The pure function `migrateRoutingState(parsed)` is exported for tests. Combined with the v2 router's runtime `invalid-state` decision for any corrupt value that slips through after migration (closes FLAG-B), I2 is enforced at two layers — disk-side scrub + runtime guard — without depending on production handlers being updated. The full type-level closure (narrowing `PendingEscalation.targetAgent` from the open `"pm" | "architect" | "design"` to `AgentId`) is queued for I8.
