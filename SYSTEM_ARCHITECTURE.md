...
**Deterministic upstream readiness gate — design + architect (2026-04-25):** `checkUpstreamReadiness(agent, specs, targetFormFactors?)` (`runtime/escalation-orchestrator.ts`) runs before the agent on every message for both design and architect paths. Design: checks PM spec only. Architect: checks PM spec first, then design spec (PM-first ordering). Uses `auditPmSpec()` and `auditDesignSpec()` — pure deterministic auditors, no LLM. If findings exist: `groupFindingsByCategory()` batches all findings by criterion, `buildCategorizedEscalationBrief()` produces a structured escalation brief with category-level recommendations and counts, and `setPendingEscalation` stores the brief. The agent never runs — design tools are stripped until upstream is clean. Design path gate: uses the same `pmSpecContent` already fetched for the informational upstream audit — no extra GitHub read. Architect path gate: reads PM spec and design spec from main before any context loading. Both paths: guarded by `!readOnly && !getPendingEscalation && !getEscalationNotification` — gate only fires when no escalation is already in progress.

**Re-audit after escalation writeback (2026-04-25):** `verifyEscalationResolution(blockingSpec, updatedSpec, requestingAgent, targetFormFactors?)` (`runtime/escalation-orchestrator.ts`) runs after every spec writeback in escalation paths. Uses the patched spec content — deterministic re-audit with `auditPmSpec()` or `auditDesignSpec()`. If findings remain: platform sets a new `pendingEscalation` with a fresh categorized brief and does NOT resume the downstream agent. The user sees "Spec partially updated, but N gaps remain. Say *yes* to bring the [agent] back." Wired in three paths: (1) design→PM escalation reply (`branch=escalation-reply`) — after `patchProductSpecWithRecommendations`, (2) architect→PM/Design escalation reply (`branch=arch-upstream-revision-reply`) — after upstream spec writeback, (3) escalation auto-close (`branch=escalation-auto-close`) — after PM branch content is applied to main via `updateApprovedSpecOnMain`. All paths clear the `escalationNotification` and return early on re-escalation.

**Escalation auto-close writeback to main (2026-04-25):** When the PM calls a save tool (`save_product_spec_draft`, `apply_product_spec_patch`, `finalize_product_spec`) during escalation continuation, the platform reads the updated spec from the PM branch and writes it to main via `updateApprovedSpecOnMain`. Invariant: approved specs live on main — human-confirmed changes to approved specs go to main. After the writeback, `verifyEscalationResolution` re-audits. If findings remain, the PM is re-escalated. If clean, design resumes.
