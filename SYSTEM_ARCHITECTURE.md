...
| **Copy completeness audit** | `runtime/spec-auditor.ts` → `auditCopyCompleteness()` | All quoted copy literals: sentence-case multi-word strings end with terminal punctuation (. ! ?); no `[TBD]`/`[placeholder]` bracket patterns. Wired into `auditSpecRenderAmbiguity()` return — blocking, same enforcement path as render ambiguity. | None (deterministic string scan — no LLM call) |
