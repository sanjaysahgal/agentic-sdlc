...
| **Phase completion gate — Design** | `runtime/phase-completion-auditor.ts` → `auditPhaseCompletion(DESIGN_RUBRIC)` | All screens defined with all states, all UI copy verbatim, all animations with timing+easing, no conflicting values, no vague language, brand token consistency, no TBD/TODO/PLACEHOLDER, no design decisions that assume product answers not explicitly in the product spec (criterion 10 — open-loop product assumption check) | **Sonnet** (runs once on approval intent, before `finalize_design_spec`; blocks phase advance until zero findings) |
